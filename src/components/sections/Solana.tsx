...
const chips = ["Anchor", "Rust", "web3.js", "Metaplex", "cNFTs", "Helius", "Jupiter", "Squads"];
          ...
          <div className="lg:sticky lg:top-28">
            <Reveal>
              ...
              <p className="mt-6 max-w-lg text-lg leading-relaxed text-mute">
                Solana work from people who read the source, not just the whitepaper.
                Sub-second finality, fees measured in fractions of a cent, and the same
                review discipline we bring to everything else.
              </p>
            ...
            <Reveal delay={140} className="mt-10">
              <CodeBlock code={sample} filename="programs/daddy_vault/src/lib.rs" />
            </Reveal>
          </div>
            ...
            {solanaCapabilities.map((cap, i) => (
              <Reveal key={cap.title} delay={i * 70}>
                <div className="card-lift card-lift-mint hairline h-full rounded-2xl bg-ink-900/50 p-6">
                  ...
                  <h3 className="mt-5 text-lg tracking-tight text-bone">{cap.title}</h3>
                  <p className="mt-2 text-[0.93rem] leading-relaxed text-mute">{cap.body}</p>
                </div>
              </Reveal>
            ))}
            ...
            <Reveal delay={430} className="sm:col-span-2">
              <div className="hairline rounded-2xl bg-gradient-to-r from-violet/[0.09] via-ink-900/60 to-mint/[0.07] p-6">
                <p className="text-[0.97rem] leading-relaxed text-bone-dim">
                  Every on-chain program we ship gets a test suite, a devnet dry run and a
                  second pair of senior eyes before it touches mainnet.
                  <span className="text-mute"> Money moves. We don&rsquo;t improvise.</span>
                </p>
              </div>
            </Reveal>
          </div>
